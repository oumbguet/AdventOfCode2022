...
class Node
  ...
  def findDeletableDirs(min)
    ...
    @next.each do |nex|
      sizes += nex.findDeletableDirs(min)
    end

    return sizes
  end

  attr_reader :prev, :next, :files, :name, :size
  attr_writer :prev, :next, :files, :name, :size
...
usedSpace = 70000000 - l.head.size
missingFreeSpace = 30000000 - usedSpace

puts "missingFreeSpace", missingFreeSpace

puts "min deletable dir", l.head.findDeletableDirs(missingFreeSpace).min() # 4370655
